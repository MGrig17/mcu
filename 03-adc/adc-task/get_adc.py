...
def read_value(ser): # Создать функцию чтения float из последовательного порта
    while True:
        try:
...
def main(): 
    ...
    try:
        while True:
            ...
            ser.write("get_adc\n".encode('ascii')) # Отправить команду get_adc в последовательный порт
            voltage_V = read_value(ser) # Считать ответ на команду get_adc c помощью написанной ранее функции read_value
